<script lang="ts">
    ...
    import { DEFAULT_GAME_INTERVAL_CLOCK_TIME, addMinuteToGameClock, addSecondToGameClock, currentInterval, gameClockTime, isBreakClockRunning, isGameClockRunning, isGameIntervalBuzzerOn, resetGameClock, startClockTime, stopClockTime, subtractMinuteFromGameClock, subtractSecondFromGameClock, totalIntervals } from "./stores/ClockStore";
</script>

<main class="h-screen w-full p-4 grid grid-rows-4 gap-y-4">
    <div class="row-span-1 grid grid-cols-10 gap-x-4">
        <GameInterval class="col-span-3" currentInterval={$currentInterval} totalIntervals={$totalIntervals} />
    ...
    <div class="row-span-3 grid grid-cols-10 gap-x-4">
        ...
        <div class="col-span-4 grid grid-rows-5 gap-y-4">
            <Button 
                class="row-span-1" 
                type={$isGameClockRunning || $isGameIntervalBuzzerOn ? "disabled" : "default"} 
                size="lg"
                text="Reset"
            ...
            <div class="row-span-1 flex gap-x-4">
                <Button 
                    class="flex-1" 
                    type={$isGameClockRunning || $isGameIntervalBuzzerOn || $gameClockTime <= 0 ? "disabled" : "default"} 
                    size="lg" 
                    text="- Min" 
                ...
                <Button 
                    class="flex-1" 
                    type={$isGameClockRunning || $isGameIntervalBuzzerOn || $gameClockTime >= DEFAULT_GAME_INTERVAL_CLOCK_TIME ? "disabled" : "default"} 
                    size="lg" 
                    text="+ Min" 
            ...
            <Button 
                class="row-span-2" 
                type={$isBreakClockRunning || $isGameIntervalBuzzerOn || $gameClockTime <= 0 ? "disabled" : "primary"} 
                size="lg" 
